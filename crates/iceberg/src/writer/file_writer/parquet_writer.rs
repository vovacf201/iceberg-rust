            ...
            for row_group in metadata.row_groups() {
                for column_chunk_metadata in row_group.columns() {
                    ...
                    *per_col_size.entry(field_id).or_insert(0) +=
                    ...
                    // A variant's two leaves describe the *same* logical values, so counting
                    // both would double every count. Attribute counts to the `value` leaf only.
                    // Bounds are skipped outright: a variant is not an orderable primitive, so
                    // the spec defines no bounds for it and the min/max aggregator rejects it.
                    if index_by_parquet_path.is_variant_field(field_id) {
                        if parquet_path.rsplit('.').next() == Some(VARIANT_VALUE_LEAF) {
