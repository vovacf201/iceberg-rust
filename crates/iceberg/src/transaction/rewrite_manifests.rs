...
use crate::spec::{
    DataFile, FormatVersion, MAIN_BRANCH, ManifestContentType, ManifestFile, ManifestListWriter,
    ManifestWriter, ManifestWriterBuilder, Operation, Snapshot, SnapshotReference,
    SnapshotRetention, Summary, DataFileFormat, update_snapshot_summaries,
};
...
fn active_files_count(manifests: &[ManifestFile]) -> u32 {
    manifests
        .iter()
        .map(|m| {
            m.added_files_count.unwrap_or(0) + m.existing_files_count.unwrap_or(0)
        })
        .sum()
}
    ...
    async fn commit(self: Arc<Self>, table: &Table) -> Result<ActionCommit> {
        ...
        let current_manifests = if let Some(snapshot) = parent_snapshot {
            let manifest_list = snapshot
                ...
                .await?;
            manifest_list.consume_entries().into_iter().collect::<Vec<_>>()
        } else {
            Vec::new()
        };
        ...
        let writer_factory = ManifestWriterFactory::new(
            table,
            commit_uuid,
            snapshot_id,
            self.key_metadata.clone(),
        ...
        let created_count = active_files_count(&new_manifests)
            + active_files_count(&self.added_manifests);
        let replaced_count = active_files_count(&rewritten_manifests)
            + active_files_count(&self.deleted_manifests);
